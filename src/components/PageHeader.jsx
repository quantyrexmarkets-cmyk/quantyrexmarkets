...
export default function PageHeader({ title }) {
  ...
  return (
    <>
      ...
      <div style={{ position: 'relative', top: 0, zIndex: 1000, background: t.navBg, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0, borderBottom: `1px solid ${t.accent}4D`, boxShadow: t.bg === '#f8fafc' ? '0 2px 8px rgba(0,0,0,0.08)' : '0 4px 24px rgba(99,102,241,0.15), 0 1px 0 rgba(99,102,241,0.05) inset' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px', alignItems: 'stretch' }}>
          ...
          {/* Balance */}
          <button type="button" style={{ padding: '5px 10px', background: t.cardBg, border: `1px solid ${t.border}`, color: t.text, fontSize: '11px', cursor: 'default', display: 'flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}>
            <span style={{ color: '#f7931a' }}>₿</span> {formatAmount(balance, user?.currency)}
          </button>

          {/* Trade */}
          <button type="button" onClick={() => navigate('/dashboard/live-trading')} style={{ padding: '5px 10px', background: 'transparent', border: '1px solid #6366f1', color: t.text, fontSize: '11px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}>
            <RefreshCw size={11}/> Trade
          </button>

          {/* Notifications */}
          <div style={{ position: 'relative' }}>
            <button type="button" onClick={() => { setShowNotifications(!showNotifications); setShowProfileMenu(false); }} style={{ background: 'none', border: 'none', color: t.text, cursor: 'pointer', padding: '5px 8px', position: 'relative', display: 'flex', alignItems: 'center' }}>
