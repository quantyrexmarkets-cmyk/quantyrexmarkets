    ...
    <div style={{ minHeight: '100vh', background: t.bg, fontFamily: "'Segoe UI', sans-serif", color: t.text }}>
      ...
      {showStopModal && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 101, background: t.cardBg, border: t.bg === '#e6e6e6' ? `1px solid ${t.border}` : '1px solid rgba(99,102,241,0.3)', borderRadius: '12px', padding: '20px', width: '320px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button type="button" onClick={() => setShowStopModal(false)} style={{ flex: 1, padding: '8px', background: t.hoverBg, border: 'none', borderRadius: '6px', color: t.text, fontSize: '9px', cursor: 'pointer' }}>Cancel</button>
              <button type="button" onClick={handleStop} style={{ flex: 1, padding: '8px', background: '#ef4444', border: 'none', borderRadius: '6px', color: t.text, fontSize: '9px', fontWeight: '700', cursor: 'pointer' }}>Stop</button>
            </div>
          </div>
        </>
      )}
      ...
      <div style={{ padding: '16px' }}>
        ...
        {loading ? (
          ...
          <div style={{ padding: '40px', textAlign: 'center' }}>
            ...
            <button type="button" onClick={() => navigate('/dashboard/copy-trading')} style={{ padding: '8px 16px', background: '#6366f1', border: 'none', borderRadius: '6px', color: t.text, fontSize: '10px', fontWeight: '700', cursor: 'pointer' }}>Browse Traders</button>
          </div>
        ) : (
          <>
            ...
            {copyTrades.map(trade => {
              ...
              return (
                <div key={trade._id} style={{ background: t.bg === '#e6e6e6' ? t.cardBg : 'linear-gradient(135deg, rgba(15,23,42,0.9), rgba(30,41,59,0.8))', border: t.bg === '#e6e6e6' ? `1px solid ${t.border}` : '1px solid rgba(99,102,241,0.3)', borderRadius: '12px', padding: '14px', marginBottom: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                    ...
                    <button type="button" onClick={() => { setSelectedTrade(trade); setShowStopModal(true); }} style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)', padding: '6px 10px', borderRadius: '6px', color: '#ef4444', fontSize: '8px', cursor: 'pointer' }}>✕ Stop</button>
                  </div>
