      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', overflowX: 'auto' }}>
        {stocks.map(s => (
          <button type="button" key={s.symbol} onClick={() => setActiveStock(s.symbol.split(':')[1])}
            style={{ padding: '6px 14px', borderRadius: '8px', background: activeStock === s.symbol.split(':')[1] ? (t.bg === '#f8fafc' ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.15)') : 'transparent', border: 'none', color: activeStock === s.symbol.split(':')[1] ? t.text : t.subText, fontSize: '11px', fontWeight: activeStock === s.symbol.split(':')[1] ? '700' : '400', cursor: 'pointer', whiteSpace: 'nowrap' }}>
            {s.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
        {periods.map(p => (
          <button type="button" key={p} onClick={() => setPeriod(p)}
            style={{ padding: '4px 10px', borderRadius: '6px', background: period === p ? (t.bg === '#f8fafc' ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.15)') : 'transparent', border: 'none', color: period === p ? t.text : t.mutedText, fontSize: '10px', fontWeight: period === p ? '700' : '400', cursor: 'pointer' }}>
            {p}
          </button>
        ))}
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, display: 'flex', fontFamily: "'Segoe UI', sans-serif", overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        {/* Top Nav */}
        <div style={{ position: 'relative', top: 0, zIndex: 1000, background: t.navBg, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0, borderBottom: `1px solid ${t.accent}4D`, boxShadow: t.bg === '#f8fafc' ? '0 2px 8px rgba(0,0,0,0.08)' : '0 4px 24px rgba(99,102,241,0.15), 0 1px 0 {t.subtleBorder} inset' }}>
