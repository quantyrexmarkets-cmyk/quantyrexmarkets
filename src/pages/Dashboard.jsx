  ...
  return (
    <div style={{ background: t.cardBg, border: '1px solid rgba(99,102,241,0.2)', borderRadius: '12px', padding: '14px', marginBottom: '16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <button type="button" onClick={() => window.location.href='/dashboard/deposit-funds'} style={{ background: '#6366f1', border: 'none', color: 'white', fontSize: '10px', fontWeight: '700', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer' }}>Deposit</button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', overflowX: 'auto' }}>
        {stocks.map(s => (
          <button type="button" key={s.symbol} onClick={() => setActiveStock(s.symbol.split(':')[1])}
            style={{ padding: '6px 14px', borderRadius: '8px', background: activeStock === s.symbol.split(':')[1] ? (t.bg === '#eef0f4' ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.15)') : 'transparent', border: 'none', color: activeStock === s.symbol.split(':')[1] ? t.text : t.subText, fontSize: '11px', fontWeight: activeStock === s.symbol.split(':')[1] ? '700' : '400', cursor: 'pointer', whiteSpace: 'nowrap' }}>
            {s.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
        {periods.map(p => (
          <button type="button" key={p} onClick={() => setPeriod(p)}
            style={{ padding: '4px 10px', borderRadius: '6px', background: period === p ? (t.bg === '#eef0f4' ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.15)') : 'transparent', border: 'none', color: period === p ? t.text : t.mutedText, fontSize: '10px', fontWeight: period === p ? '700' : '400', cursor: 'pointer' }}>
            {p}
          </button>
        ))}
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, display: 'flex', fontFamily: "'Segoe UI', sans-serif", overflow: 'hidden' }}>
      ...
      {tradeSuccess && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 151, background: 'white', padding: '28px 20px', width: '260px', textAlign: 'center', borderRadius: '10px' }}>
            ...
            <button type="button" onClick={() => setTradeSuccess(false)} style={{ padding: '8px 28px', background: '#6366f1', border: 'none', color: 'white', fontSize: '10px', fontWeight: '600', }}>Okay</button>
          </div>
        </>
      )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        {/* Top Nav */}
        <div style={{ position: 'sticky', top: 0, zIndex: 1000, background: t.navBg, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0, borderBottom: `1px solid ${t.accent}4D`, boxShadow: t.bg === '#eef0f4' ? 'none' : '0 4px 24px rgba(99,102,241,0.15), 0 1px 0 {t.subtleBorder} inset' }}>
                      ...
                      {showNotifications && (
                        <>
                          ...
                          <div style={{ position: 'absolute', top: '110%', right: 0, background: t.dropdownBg, border: `1px solid ${t.border}`, zIndex: 999, minWidth: '260px', boxShadow: '0 8px 24px rgba(0,0,0,0.4)', borderRadius: '8px', overflow: 'hidden' }}>
                            <div style={{ padding: '12px 16px', borderBottom: `1px solid ${t.subtleBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            {(notifications.length ? notifications.slice(0,5) : [
                              ...
                              <div key={i} style={{ padding: '10px 16px', borderBottom: `1px solid ${t.tableRowBorder}`, background: n.unread ? 'rgba(99,102,241,0.08)' : 'transparent', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                                ...
                                <div style={{ flex: 1 }}>
                                  ...
                                  <div style={{ color: t.mutedText, fontSize: '9px', marginBottom: '2px' }}>{n.desc}</div>
                                  <div style={{ color: t.faintText, fontSize: '8px' }}>{typeof n.time === 'string' && n.time.includes('ago') ? n.time : new Date(n.time).toLocaleDateString()}</div>
                                </div>
              ...
              {showProfileMenu && (
                <>
                  ...
                  <div style={{ position: 'absolute', top: '110%', right: 0, background: t.dropdownBg, border: `1px solid ${t.border}`, zIndex: 999, minWidth: '180px', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
                    <div onClick={() => { navigate('/dashboard/profile'); setShowProfileMenu(false); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', cursor: 'pointer', color: t.overlayText, fontSize: '11px' }}
                      onMouseEnter={e => e.currentTarget.style.background=t.hoverBg}
                      onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                      <User size={14} /> My Account
                    </div>
                    <div onClick={() => { navigate('/dashboard/profile', { state: { tab: 'edit' } }); setShowProfileMenu(false); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', cursor: 'pointer', color: t.overlayText, fontSize: '11px' }}
                      onMouseEnter={e => e.currentTarget.style.background=t.hoverBg}
                      onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                      <Settings size={14} /> Edit Account
                    </div>
                    <div onClick={() => { navigate('/dashboard/change-password'); setShowProfileMenu(false); }} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', cursor: 'pointer', color: t.overlayText, fontSize: '11px' }}
                      onMouseEnter={e => e.currentTarget.style.background=t.hoverBg}
                      onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                      <Lock size={14} /> Change Password
                    </div>
                    <div style={{ height: '1px', background: t.subtleBorder }} />
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px', minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '16px', marginTop: '8px' }}>
              ...
              <span style={{ color: t.text, fontSize: '13px', fontWeight: '300', letterSpacing: '1px', fontFamily: 'inherit' }}>QUANTYREX</span><span style={{ color: '#6366f1', fontSize: '13px', fontWeight: '300', letterSpacing: '1px', fontFamily: 'inherit' }}>&nbsp;&nbsp;MARKETS</span>
              <span style={{ color: t.mutedText, fontSize: '10px' }}>| {u.firstName || ''} {u.lastName || ''}</span>
            </div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <div style={{ background: t.cardBg2, border: `1px solid ${t.border}`, borderRadius: '12px', padding: '14px', marginBottom: '8px', position: 'relative', overflow: 'hidden', boxShadow: t.bg === '#111111' ? '0 0 20px rgba(99,102,241,0.15), inset 0 1px 0 rgba(99,102,241,0.1)' : 'none' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', marginBottom: '4px' }}>
                    <svg width='16' height='16' fill='none' stroke={t.subText} viewBox='0 0 24 24' strokeWidth='2'><rect x='2' y='5' width='20' height='14' rx='2'/><line x1='2' y1='10' x2='22' y2='10'/></svg>
                  ...
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: t.border, padding: '4px 12px', borderRadius: '20px', marginBottom: '8px' }}>
                    <svg width='12' height='12' fill='none' stroke={t.subText} viewBox='0 0 24 24' strokeWidth='2'><polyline points='20 6 9 17 4 12'/></svg>
                    <span style={{ color: t.subText, fontSize: '11px' }}>Available for Withdrawal</span>
                  </div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button type="button" onClick={() => navigate('/dashboard/withdraw')} style={{ flex: 1, background: t.cardBg2, border: `1px solid ${t.border}`, color: t.text, fontSize: '11px', fontWeight: '600', padding: '10px', cursor: 'pointer', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
              {Array.isArray(stats) && stats.map((s, i) => (
                <div key={i} style={{ position: 'relative', overflow: 'hidden', background: t.cardBg, border: `1px solid ${t.tableOuterBorder}`, borderRadius: '14px', padding: '12px 10px', minHeight: '90px', boxShadow: t.bg === '#eef0f4' ? 'none' : '0 4px 24px rgba(0,0,0,0.2)' }}>
                  {/* Gradient overlay */}
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      {s.hasViewTrade && <button type="button" onClick={() => navigate('/dashboard/packages')} style={{ background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.3)', color: '#818cf8', fontSize: '8px', padding: '2px 6px', cursor: 'pointer', borderRadius: '4px' }}>View</button>}
                    </div>
                  </div>
                </div>
              ))}
            ...
            {u.kycStatus !== 'approved' && (
              <div style={{ background: t.cardBg, border: `1px solid ${t.tableOuterBorder}`, borderRadius: '16px', padding: '20px', marginBottom: '16px', textAlign: 'center', boxShadow: t.bg === '#eef0f4' ? 'none' : '0 4px 24px rgba(0,0,0,0.2)' }}>
                ...
                <div style={{ color: t.subText, fontSize: '12px', marginBottom: '16px' }}>Complete verification to access all features</div>
                <button type="button" onClick={() => navigate('/dashboard/kyc')} style={{ width: '100%', background: 'linear-gradient(90deg, #3b82f6, #6366f1)', border: 'none', color: 'white', fontSize: '13px', fontWeight: '700', padding: '13px', cursor: 'pointer', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', boxShadow: '0 4px 15px rgba(99,102,241,0.4)' }}>
                  View Details
            ...
            <div style={{ background: t.glassBg, border: `1px solid ${t.glassBorder}`, borderRadius: '12px', padding: '10px', boxShadow: '0 4px 24px rgba(0,0,0,0.05)', marginBottom: '20px' }}>
              ...
              <div style={{ border: `1px solid ${t.tableDivider}` }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 10px', borderTop: `1px solid ${t.tableRowBorder}` }}>
                  <span style={{ color: t.faintText, fontSize: '8px' }}>Showing {Math.min(transactions.length, 10)} of {transactions.length} entries</span>
                  <div style={{ display: 'flex', gap: '4px' }}>
                    <button type="button" style={{ background: t.paginationBg, border: `1px solid ${t.paginationBorder}`, color: t.paginationText, fontSize: '10px', padding: '2px 8px', }}>&#8249;</button>
                    <button type="button" style={{ background: t.paginationBg, border: `1px solid ${t.paginationBorder}`, color: t.paginationText, fontSize: '10px', padding: '2px 8px', }}>&#8250;</button>
                  </div>
                </div>
              </div>
        ...
        <div style={{ padding: '0 12px 16px' }}>
          <div style={{ background: 'linear-gradient(135deg, #6366f1, #4f46e5)', borderRadius: '12px', padding: '20px 16px', textAlign: 'center', }}>
            ...
            <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: '10px' }}>Start a new trade instantly or explore investment plans.</div>
          </div>
        </div>
          ...
          <div style={{ background: t.glassBg, border: `1px solid ${t.glassBorder}`, borderRadius: '12px', padding: '14px', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
              {[
                ...
                <div key={i}>
                  <label style={{ color: t.subText, fontSize: '9px', display: 'flex', alignItems: 'center', gap: '3px', marginBottom: '4px' }}>
