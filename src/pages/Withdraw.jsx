        ...
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 151, background: t.bg, padding: '20px', width: '340px', borderRadius: '4px', border: `1px solid ${t.border}`, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <div>
                <span style={{ color: t.text, fontSize: '11px', fontWeight: '700' }}>Max Withdrawal: </span><span style={{ color: '#22c55e', fontSize: '11px', fontWeight: '700' }}>{currencySymbol}{(user?.minimumWithdrawal||100).toLocaleString()}</span>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 151, background: t.bg, padding: '20px', width: '320px', borderRadius: '4px', border: `1px solid ${t.border}`, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <button type="button" onClick={() => { setShowForm(false); }} style={{ background: 'none', border: 'none', color: '#818cf8', fontSize: '9px', cursor: 'pointer' }}>← Back</button>
                <span style={{ color: t.text, fontSize: '11px', fontWeight: '700' }}>{methods.find(m => m.id === selected)?.label}</span>
              </div>
              <button type="button" onClick={() => { setShowForm(false); setShowMethodSelector(false); }} style={{ background: 'none', border: 'none', color: t.subText, cursor: 'pointer', fontSize: '18px' }}>×</button>
            </div>
              ...
              <>
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                    {['BTC','ETH','USDT','BNB','XRP','SOL'].map(c => <button type="button" key={c} onClick={() => setCoin(c)} style={{ padding: '4px 8px', background: coin === c ? '#6366f1' : t.subtleBg, border: 'none', color: 'white', fontSize: '7px', fontWeight: '700', cursor: 'pointer' }}>{c}</button>)}
                  </div>
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <label style={labelStyle}>Select Network</label>
                  <div style={{ display: 'flex', gap: '5px' }}>
                    {['TRC20','ERC20','BEP20'].map(n => <button type="button" key={n} onClick={() => setNetwork(n)} style={{ padding: '4px 8px', background: network === n ? '#6366f1' : t.subtleBg, border: 'none', color: 'white', fontSize: '7px', fontWeight: '700', cursor: 'pointer' }}>{n}</button>)}
                  </div>
                </div>
                <div style={{ marginBottom: '12px' }}>
            ...
            <button type="button" onClick={() => { if (validate()) setShowConfirm(true); }} style={{ width: '100%', padding: '10px', background: '#6366f1', border: 'none', color: 'white', fontSize: '10px', fontWeight: '700', cursor: 'pointer' }}>
              REQUEST WITHDRAWAL
            </button>
          </div>
        </>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 201, background: 'white', padding: '24px 22px', width: '320px', maxHeight: '85vh', overflowY: 'auto', textAlign: 'center', borderRadius: '10px', fontFamily: 'inherit' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button type="button" onClick={handleConfirm} style={{ flex: 1, padding: '10px', background: '#6366f1', border: 'none', color: 'white', fontSize: '9px', fontWeight: '600', cursor: 'pointer' }}>Confirm</button>
            </div>
          </div>
        </>
      )}
      ...
      {showSuccess && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 201, background: 'white', padding: '24px 22px', width: '320px', maxHeight: '85vh', overflowY: 'auto', textAlign: 'center', borderRadius: '10px', fontFamily: 'inherit' }}>
            ...
            <button type="button" onClick={() => setShowSuccess(false)} style={{ padding: '10px 28px', background: '#6366f1', border: 'none', color: 'white', fontSize: '11px', fontWeight: '700', cursor: 'pointer', borderRadius: '6px' }}>Check History</button>
          </div>
        </>
      )}
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={i} style={{ borderBottom: `1px solid ${t.tableRowBorder}`, background: i % 2 === 0 ? 'transparent' : t.subtleBg }}>
                  <td style={{ padding: '8px 10px', color: t.text, fontSize: '8px', fontWeight: '700', borderRight: `1px solid ${t.subtleBorder}` }}>-{formatAmount(w.amount, user?.currency)}</td>
