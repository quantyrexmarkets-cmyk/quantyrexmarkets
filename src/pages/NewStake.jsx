  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, display: 'flex', flexDirection: 'column', fontFamily: "'Segoe UI', sans-serif", color: t.text }}>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 151, background: 'white', padding: '36px 28px', width: '320px', textAlign: 'center', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button type="button" onClick={() => { setShowSuccess(false); setSelected(null); setAmount(''); }} style={{ flex: 1, padding: '8px', background: t.subtleBg, border: `1px solid ${t.border}`, color: t.text, fontSize: '9px', cursor: 'pointer' }}>New Stake</button>
              <button type="button" onClick={() => navigate('/dashboard/stake')} style={{ flex: 1, padding: '8px', background: '#6366f1', border: 'none', color: 'white', fontSize: '9px', fontWeight: '600', cursor: 'pointer' }}>View History</button>
            </div>
          </div>
        </>
      )}
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 151, background: 'white', padding: '36px 28px', width: '320px', textAlign: 'center', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button type="button" onClick={() => { setShowInsufficient(false); navigate('/dashboard/deposit'); }} style={{ flex: 1, padding: '8px', background: '#6366f1', border: 'none', color: 'white', fontSize: '9px', fontWeight: '600', cursor: 'pointer' }}>Deposit Now</button>
            </div>
          </div>
        </>
      )}
      ...
      <div style={{ padding: '16px' }}>
        ...
        {!selected ? (
          <>
            <div style={{ color: t.subText, fontSize: '9px', marginBottom: '12px' }}>Select a crypto asset to stake</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {cryptoPlans.map((c, i) => (
                <div key={i} onClick={() => setSelected(c)}
                  ...
                  {/* Info */}
                  <div style={{ flex: 1 }}>
                    <div style={{ color: t.text, fontSize: '13px', fontWeight: '700', marginBottom: '2px' }}>{c.name}</div>
                    <div style={{ color: t.subText, fontSize: '10px', marginBottom: '4px' }}>{c.symbol}</div>
                    <div style={{ color: '#22c55e', fontSize: '10px', fontWeight: '600' }}>Minimum ROI: {c.roi}</div>
                  </div>
                  {/* Button */}
                  <button type="button" style={{ background: t.border, border: `1px solid ${t.border}`, color: t.text, fontSize: '9px', padding: '7px 14px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
          ...
          /* Stake Form */
          <div>
            <button type="button" onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', color: '#6366f1', fontSize: '9px', cursor: 'pointer', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '4px' }}>
              ← Back to coins
            </button>
            ...
            <div style={{ background: t.cardBg, border: `1px solid ${selected.color}30`, padding: '16px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <div>
                ...
                <div style={{ color: t.subText, fontSize: '10px' }}>{selected.symbol}</div>
            ...
            <div style={{ background: t.cardBg, border: '1px solid rgba(99,102,241,0.2)', padding: '16px' }}>
              ...
              {/* Duration */}
              <div style={{ color: t.subText, fontSize: '8px', marginBottom: '6px' }}>Lock Duration</div>
              <div style={{ display: 'flex', gap: '6px', marginBottom: '14px' }}>
                {[7, 14, 30, 60, 90].map(d => (
              ...
              <div style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', padding: '10px 12px', marginBottom: '14px', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                ...
                <div style={{ color: 'rgba(255,100,100,0.8)', fontSize: '8px', lineHeight: '1.6' }}>
                  Funds will be locked for <strong style={{ color: t.text }}>{duration} days</strong>. Early withdrawal is not permitted. Profits are credited daily.
                </div>
              </div>

              {error && <div style={{ color: '#ef4444', fontSize: '8px', marginBottom: '10px' }}>{error}</div>}

              <button type="button" onClick={handleStake} disabled={submitting}
