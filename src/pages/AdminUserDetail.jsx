...
export default function AdminUserDetail() {
  ...
  const setMinWithdrawal = async (id) => {
    const amt = prompt('Set minimum withdrawal amount (in USD):');
    if (!amt || isNaN(amt)) return;
    await api(`/users/${id}/minimum-withdrawal`, 'PUT', { minimumWithdrawal: parseFloat(amt) });
    setSelectedUser(prev => ({ ...prev, minimumWithdrawal: parseFloat(amt) }));
    showMsg('Min withdrawal updated to 
