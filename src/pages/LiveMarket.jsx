  ...
  useEffect(() => {
    if (activeTab === 'chart' && chartRef.current && !chartLoaded.current) {
      ...
      script.innerHTML = JSON.stringify({
        ...
        style: '1',
        locale: 'en',
        backgroundColor: t.bg === '#e6e6e6' ? '#e6e6e6' : t.bg === '#111111' ? '#111111' : '#0f172a',
        ...
        <div style={{ display: 'flex', marginBottom: '14px', borderBottom: `1px solid ${t.border}` }}>
          {[
            ...
            <button type="button" key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
              background: activeTab === tab.id ? (t.bg === '#e6e6e6' ? 'rgba(99,102,241,0.1)' : 'rgba(99,102,241,0.15)') : 'transparent',
