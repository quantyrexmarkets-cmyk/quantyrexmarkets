    ...
    <div style={{ minHeight: '100vh', background: t.bg, fontFamily: "'Segoe UI', sans-serif", color: t.text, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', overflowX: 'auto' }}>
          {SYMBOLS.map(s => (
            <button type="button" key={s.label} onClick={() => setSymbol(s)}
        ...
        <button type="button" onClick={handleTrade} disabled={submitting}
      ...
      {showError && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 151, background: 'white', padding: '36px 28px', width: '320px', textAlign: 'center', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button type="button" onClick={() => setShowError(false)} style={{ flex: 1, padding: '8px', background: t.subtleBg, border: `1px solid ${t.border}`, color: t.text, fontSize: '9px', cursor: 'pointer' }}>Cancel</button>
              {error === 'Insufficient balance' && (
                <button type="button" onClick={() => { setShowError(false); navigate('/dashboard/deposit'); }} style={{ flex: 1, padding: '8px', background: '#6366f1', border: 'none', color: 'white', fontSize: '9px', fontWeight: '600', cursor: 'pointer' }}>Deposit Now</button>
              )}
            </div>
          </div>
        </>
