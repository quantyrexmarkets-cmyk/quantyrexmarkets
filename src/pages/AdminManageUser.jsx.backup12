      ...
      <div style={{ maxWidth:'600px', margin:'0 auto', padding:'16px' }}>
        ...
        <S title="Registration Fee">
          ...
          <div style={{ display:'flex', gap:'6px' }}>
            ...
            <button onClick={async()=>{const v=parseFloat(regFeeRef.current?.value||0);if(!v)return showMsg('Enter amount');const r=await api('/users/'+id+'/registration-fee','PUT',{required:true,amount:v});setUser(r.user);showMsg('Set');if(regFeeRef.current)regFeeRef.current.value='';}} style={{ padding:'7px 12px', background:'transparent', border:'1.5px solid '+t.tableDivider, color:t.text, fontSize:'10px', fontWeight:'600', cursor:'pointer', borderRadius:'6px' }}>Set</button>
        ...
        <S title="Add Fee">
          ...
          <button onClick={async()=>{const fa=parseFloat(feeAmountRef.current?.value||0);if(!fa||fa<=0)return showMsg('Enter amount');const fl=feeLabelRef.current?.value||'';const fd=feeDescRef.current?.value||'';const r=await api('/users/'+id+'/fees','POST',{type:feeType,label:fl||feeType,amount:fa,description:fd});setUser(r.user);showMsg('Fee added');if(feeLabelRef.current)feeLabelRef.current.value='';if(feeAmountRef.current)feeAmountRef.current.value='';if(feeDescRef.current)feeDescRef.current.value='';}} style={{ width:'100%', padding:'9px', background:'transparent', border:'1.5px solid '+t.tableDivider, color:t.text, fontSize:'10px', fontWeight:'600', cursor:'pointer', borderRadius:'6px', display:'flex', alignItems:'center', justifyContent:'center', gap:'4px', marginBottom:'8px' }}>+ Add Fee & Notify</button>
