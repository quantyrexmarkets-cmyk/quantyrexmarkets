...
export default function AdminManageUser() {
  ...
  useEffect(() => {
    ...
    if (location.state?.user) {
      ...
      setUser(u);
      setBalance(Number(convertAmount(u.balance||0, u.currency)).toLocaleString());
    ...
    api('/users/' + id).then(u => {
      ...
      if (balanceRef.current && document.activeElement !== balanceRef.current) {
        balanceRef.current.value = Number(convertAmount(u.balance||0, u.currency)).toLocaleString();
      }
      if (profitRef.current && document.activeElement !== profitRef.current) {
        profitRef.current.value = Number(convertAmount(u.totalProfit||0, u.currency)).toLocaleString();
      }
  ...
  return (
    <div style={{ minHeight:'100vh', background:t.bg, fontFamily:"'Segoe UI',sans-serif", color:t.text }}>
      ...
      <div style={{ maxWidth:'600px', margin:'0 auto', padding:'16px' }}>
        ...
        <S title="Balance" t={t}>
          <input ref={balanceRef} defaultValue={Number(convertAmount(user.balance||0, user.currency)).toLocaleString()} key={`bal-${user._id||user.id}`} placeholder={`Current: ${getCurrencySymbol(user.currency)}${Number(convertAmount(user.balance||0, user.currency)).toLocaleString()} (${getCurrencyCode(user.currency)})`} type="text" inputMode="numeric" onInput={formatNumberInput} style={inp}/>
          <div style={{ color:t.subText, fontSize:'8px', marginTop:'2px' }}>Enter value in {getCurrencyCode(user.currency)} · stored as USD</div>
          {btn(async()=>{const rawVal=parseFloat(String(balanceRef.current?.value||'0').replace(/,/g, ''));const code=getCurrencyCode(user.currency);const rates=JSON.parse(localStorage.getItem('fx_rates')||'{}');const rate=rates[code]||1;const usdVal=rawVal/rate;const r=await api('/users/'+id+'/balance','PUT',{balance:usdVal});if(r.user){setUser(r.user);if(balanceRef.current)balanceRef.current.value=convertAmount(r.user.balance||0, r.user.currency);} showMsg('Balance updated');}, 'Set Balance', <DollarSign size={12}/>)}
          <div style={{ marginTop:'8px', color:t.subText, fontSize:'9px', marginBottom:'4px' }}>Profit</div>
          <input ref={profitRef} defaultValue={Number(convertAmount(user.totalProfit||0, user.currency)).toLocaleString()} key={`profit-${user._id||user.id}`} placeholder={`Current: ${getCurrencySymbol(user.currency)}${Number(convertAmount(user.totalProfit||0, user.currency)).toLocaleString()} (${getCurrencyCode(user.currency)})`} type="text" inputMode="numeric" onInput={formatNumberInput} style={inp}/>
          <div style={{ color:t.subText, fontSize:'8px', marginTop:'2px' }}>Enter value in {getCurrencyCode(user.currency)} · stored as USD</div>
            ...
            <>
              ...
              <div style={{ display:'flex', gap:'6px', marginBottom:'8px' }}>
                ...
                <button type="button" disabled={clicked==='regfee-approve'} onClick={async()=>{
                  const localVal = parseFloat(String(regFeeRef.current?.value||'0').replace(/,/g, ''));
        ...
        <S title="Add Fee" t={t}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'6px', marginBottom:'6px' }}>
            ...
            <input type="text" inputMode="numeric" onInput={formatNumberInput} ref={feeAmountRef} defaultValue="" placeholder={`Amount in ${getCurrencyCode(user.currency)}`} style={{ background:t.inputBg, border:`1px solid ${t.border}`, color:t.text, fontSize:'10px', padding:'7px', outline:'none', borderRadius:'6px', boxSizing:'border-box' }}/>
          </div>
          ...
          <button type="button" disabled={clicked==='addfee'} onClick={async()=>{const localVal=parseFloat(String(feeAmountRef.current?.value||'0').replace(/,/g, ''));if(!localVal||localVal<=0)return showMsg('Enter amount');const code=getCurrencyCode(user.currency);const rates=JSON.parse(localStorage.getItem('fx_rates')||'{}');const rate=rates[code]||1;const fa=localVal/rate;const fl=feeLabelRef.current?.value||'';const fd=feeDescRef.current?.value||'';setClicked('addfee');try{const r=await api('/users/'+id+'/fees','POST',{type:feeType,label:fl||feeType,amount:fa,localAmount:localVal,localCurrency:code,description:fd});setUser(r.user);showMsg('Fee added');if(feeLabelRef.current)feeLabelRef.current.value='';if(feeAmountRef.current)feeAmountRef.current.value='';if(feeDescRef.current)feeDescRef.current.value='';}finally{setTimeout(()=>setClicked(''),500);}}} style={{ width:'100%', padding:'9px', background:clicked==='addfee'?'rgba(99,102,241,0.15)':'transparent', border:'1.5px solid '+(clicked==='addfee'?'#6366f1':t.tableDivider), color:clicked==='addfee'?'#6366f1':t.text, fontSize:'10px', fontWeight:'600', cursor:clicked==='addfee'?'wait':'pointer', borderRadius:'6px', display:'flex', alignItems:'center', justifyContent:'center', gap:'4px', marginBottom:'8px' }}>{clicked==='addfee'?(<><span style={{animation:'dotPulse 1.4s infinite',animationDelay:'0s'}}>.</span><span style={{animation:'dotPulse 1.4s infinite',animationDelay:'0.2s'}}>.</span><span style={{animation:'dotPulse 1.4s infinite',animationDelay:'0.4s'}}>.</span></>):'+ Add Fee & Notify'}</button>
        ...
        <S title="Account Upgrade" t={t}>
          ...
          <div style={{ padding:'12px 14px', background:t.bg, borderRadius:'8px', marginBottom:'12px', display:'flex', justifyContent:'space-between', alignItems:'center', border:`1px solid ${user.accountUpgraded?'rgba(34,197,94,0.3)':t.border}` }}>
            ...
            <div style={{ textAlign:'right' }}>
              ...
              <div style={{ fontSize:'15px', fontWeight:'700', color:t.text, marginTop:'4px' }}>{getCurrencySymbol(user.currency)}{Number(convertAmount(user.minimumWithdrawal||100, user.currency)).toLocaleString()}</div>
            </div>
          </div>
