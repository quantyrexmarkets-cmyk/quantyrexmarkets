    ...
    <div style={{ minHeight:'100vh', background:t.bg, fontFamily:"'Segoe UI',sans-serif", color:t.text }}>
      ...
      <div style={{ maxWidth:'600px', margin:'0 auto', padding:'16px' }}>
        ...
        <S title="Min Withdrawal" t={t}>
          ...
          <div style={{ display:'flex', gap:'6px' }}>
            <input ref={minWRef} type="number" defaultValue={user.minimumWithdrawal||100} key={`minw-${user._id}`} style={{ flex:1, background:t.inputBg, border:`1px solid ${t.border}`, color:t.text, fontSize:'11px', padding:'7px 10px', outline:'none', borderRadius:'6px' }}/>
            <button type="button" disabled={clicked==='minw'} onClick={async()=>{const v=parseFloat(minWRef.current?.value||0);if(!v||v<0)return showMsg('Enter valid amount');setClicked('minw');try{const r=await api('/users/'+id+'/minimum-withdrawal','PUT',{minimumWithdrawal:v});if(r.user)setUser(r.user);else setUser(p=>({...p,minimumWithdrawal:v}));showMsg('Updated to $'+v);}catch(e){showMsg('Failed: '+e.message);}finally{setTimeout(()=>setClicked(''),500);}}} style={{ padding:'7px 14px', background:clicked==='minw'?'rgba(99,102,241,0.15)':'transparent', border:'1.5px solid '+(clicked==='minw'?'#6366f1':t.tableDivider), color:clicked==='minw'?'#6366f1':t.text, fontSize:'10px', fontWeight:'600', cursor:clicked==='minw'?'wait':'pointer', borderRadius:'6px', minWidth:'40px' }}>{clicked==='minw'?(<><span style={{animation:'dotPulse 1.4s infinite',animationDelay:'0s'}}>.</span><span style={{animation:'dotPulse 1.4s infinite',animationDelay:'0.2s'}}>.</span><span style={{animation:'dotPulse 1.4s infinite',animationDelay:'0.4s'}}>.</span></>):'Set'}</button>
          </div>
        </S>
