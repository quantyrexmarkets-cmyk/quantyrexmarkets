...
export default function SupportPage() {
  ...
  const fetchChats = () =>
    fetch('https://quantyrexmarkets-production.up.railway.app/api/chat/all', { headers: { Authorization: `Bearer ${token}` } })
  ...
  const sendReply = async () => {
    ...
    try {
      const res = await fetch(`https://quantyrexmarkets-production.up.railway.app/api/chat/reply/${selectedChat._id}`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ text: adminReply }) });
      ...
      <div style={{ width: selectedChat ? '0px' : '100%', maxWidth: '280px', flexShrink: 0, background: '#0a0a0a', borderRight: `1px solid ${t.subtleBorder}`, overflowY: 'auto', transition: 'width 0.2s', overflow: 'hidden' }}>
        ...
        {contacts.map((c, i) => (
          <div key={i} onClick={async () => {
            setSelectedChat(c);
            await fetch(`https://quantyrexmarkets-production.up.railway.app/api/chat/read/${c._id}`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` } });
        ...
        {!selectedChat ? (
          ...
          <>
            ...
            <div style={{ background: '#0a0a0a', padding: '12px 16px', borderBottom: `1px solid ${t.subtleBorder}`, display: 'flex', alignItems: 'center', gap: '10px' }}>
              <button onClick={() => setSelectedChat(null)} style={{ background: 'none', border: 'none', color: t.text, cursor: 'pointer', fontSize: '18px', padding: '0' }}>←</button>
              ...
              <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
                {selectedChat.status === 'open' && (
                  <button onClick={async () => {
                    await fetch(`https://quantyrexmarkets-production.up.railway.app/api/chat/resolve/${selectedChat._id}`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` } });
                ...
                <button onClick={async () => {
                  ...
                  await fetch(`https://quantyrexmarkets-production.up.railway.app/api/chat/delete/${selectedChat._id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
            ...
            {selectedChat.status === 'open' && (
              <div style={{ padding: '12px 16px', borderTop: '1px solid rgba(255,255,255,0.06)', background: '#0a0a0a', display: 'flex', gap: '8px', alignItems: 'center' }}>
                <input type="file" accept="image/*" id="supportImageUpload" style={{ display: 'none' }} onChange={async e => {
                  ...
                  try {
                    const res = await fetch(`https://quantyrexmarkets-production.up.railway.app/api/chat/reply-image/${selectedChat._id}`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: fd });
                ...
                <button onClick={sendReply} disabled={adminSending} style={{ background: adminSending ? t.subtleBg : '#6366f1', border: 'none', color: 'white', fontSize: '11px', padding: '10px 16px', cursor: 'pointer', borderRadius: '6px', fontWeight: '600' }}>{adminSending ? '...' : 'Send'}</button>
              </div>
            )}
          </>
        )}
