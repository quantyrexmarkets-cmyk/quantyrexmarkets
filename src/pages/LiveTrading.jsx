    ...
    <div style={{ minHeight: '100vh', background: t.bg, display: 'flex', flexDirection: 'column', fontFamily: "'Segoe UI', sans-serif", color: t.text }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, display: 'flex', flexDirection: 'column', background: t.bg }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: t.cardBg, borderBottom: `1px solid ${t.border}`, flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <button type="button" onClick={() => setShowForm(false)} style={{ background: 'none', border: 'none', color: t.text, cursor: 'pointer', fontSize: '20px', lineHeight: 1 }}>←</button>
              <div>
            ...
            <div style={{ display: 'flex', gap: '4px', overflowX: 'auto' }}>
              {SYMBOLS.map(s => (
                <button type="button" key={s.label} onClick={() => setSymbol(s)} style={{ padding: '3px 8px', background: symbol.label === s.label ? '#6366f1' : t.subtleBg, border: `1px solid ${symbol.label === s.label ? '#6366f1' : t.border}`, color: symbol.label === s.label ? 'white' : t.subText, fontSize: '8px', fontWeight: '700', cursor: 'pointer', borderRadius: '4px', whiteSpace: 'nowrap' }}>
                  {s.label.split('/')[0]}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '8px 12px', background: t.cardBg2, borderBottom: `1px solid ${t.border}`, flexShrink: 0, overflowX: 'auto' }}>
            ...
            {INTERVALS.map(iv => (
              <button type="button" key={iv.value} onClick={() => setTvInterval(iv.value)} style={{ padding: '2px 8px', background: tvInterval === iv.value ? '#6366f1' : 'transparent', border: `1px solid ${tvInterval === iv.value ? '#6366f1' : t.border}`, color: tvInterval === iv.value ? 'white' : t.subText, fontSize: '8px', fontWeight: '700', cursor: 'pointer', borderRadius: '4px' }}>
                {iv.label}
              </button>
            ))}
            {currentPrice && (
              ...
              {bottomTab === 'trade' && (
                <div>
                  <div style={{ display: 'flex', gap: '6px', marginBottom: '8px' }}>
                    <button type="button" onClick={() => setDirection('buy')} style={{ flex: 1, padding: '8px', background: direction === 'buy' ? '#16a34a' : t.subtleBg, border: `2px solid ${direction === 'buy' ? '#16a34a' : t.border}`, color: direction === 'buy' ? 'white' : t.text, fontSize: '11px', fontWeight: '800', cursor: 'pointer', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                      <ArrowUpCircle size={14}/> BUY
                    </button>
                    <button type="button" onClick={() => setDirection('sell')} style={{ flex: 1, padding: '8px', background: direction === 'sell' ? '#dc2626' : t.subtleBg, border: `2px solid ${direction === 'sell' ? '#dc2626' : t.border}`, color: direction === 'sell' ? 'white' : t.text, fontSize: '11px', fontWeight: '800', cursor: 'pointer', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                      <ArrowDownCircle size={14}/> SELL
                    </button>
                  </div>
                  <div style={{ display: 'flex', gap: '4px', marginBottom: '8px' }}>
                    {QUICK_AMOUNTS.map(a => (
                      <button type="button" key={a} onClick={() => setAmount(String(a))} style={{ flex: 1, padding: '4px', background: amount === String(a) ? '#6366f1' : t.subtleBg, border: `1px solid ${amount === String(a) ? '#6366f1' : t.border}`, color: amount === String(a) ? 'white' : t.subText, fontSize: '8px', cursor: 'pointer', borderRadius: '4px' }}>${a}</button>
                    ))}
                  </div>
                  ...
                  <button type="button" onClick={handleTrade} disabled={submitting} style={{ width: '100%', padding: '11px', background: direction === 'buy' ? '#16a34a' : '#dc2626', border: 'none', color: 'white', fontSize: '12px', fontWeight: '800', cursor: 'pointer', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <button type="button" onClick={() => setShowForm(true)} style={{ background: '#6366f1', border: 'none', color: 'white', fontSize: '9px', fontWeight: '700', padding: '8px 14px', cursor: 'pointer' }}>
            + New Trade
          </button>
        </div>
        ...
        <div style={{ background: t.cardBg, border: `1px solid ${t.tableOuterBorder}` }}>
          <div style={{ display: 'flex', borderBottom: `1px solid ${t.border}` }}>
            {[['open','Open',openTrades.length],['closed','Closed',closedTrades.length]].map(([key,label,count]) => (
              <button type="button" key={key} onClick={() => { setTab(key); setPage(1); }} style={{ flex: 1, padding: '10px', background: tab === key ? (t.bg === '#f8fafc' ? 'rgba(99,102,241,0.08)' : 'rgba(99,102,241,0.15)') : 'transparent', border: 'none', borderBottom: tab === key ? '2px solid #6366f1' : '2px solid transparent', color: tab === key ? '#6366f1' : t.mutedText, fontSize: '9px', fontWeight: tab === key ? '700' : '400', cursor: 'pointer' }}>
              ...
              {paginated.length === 0 ? (
                ...
                <tr key={i} style={{ borderBottom: `1px solid ${t.tableRowBorder}`, background: i%2===0 ? 'transparent' : t.tableAltRow }}>
                  ...
                  {tab === 'open' ? (<>
                    ...
                    <td style={{ padding: '8px 6px' }}><button type="button" onClick={fetchAll} style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', color: '#ef4444', fontSize: '6px', padding: '2px 5px', cursor: 'pointer', borderRadius: '3px' }}>Close</button></td>
                  </>) : (<>
