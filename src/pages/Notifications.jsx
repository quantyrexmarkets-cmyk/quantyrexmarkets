...
export default function Notifications() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: t.bg, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px', borderBottom: `1px solid ${t.border}`, position: 'relative', top: 0, zIndex: 100 }}>
        ...
        <button type="button" onClick={markAllRead} style={{ marginLeft: 'auto', background: 'none', border: '1px solid rgba(99,102,241,0.4)', color: '#6366f1', fontSize: '9px', padding: '4px 10px', cursor: 'pointer', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '4px' }}>
      ...
      <div style={{ display: 'flex', gap: '0', borderBottom: `1px solid ${t.border}`, padding: '0 16px' }}>
        {['all', 'unread'].map(f => (
          <button type="button" key={f} onClick={() => setFilter(f)} style={{ border: 'none', color: filter === f ? '#6366f1' : t.mutedText, fontSize: '11px', padding: '10px 16px', cursor: 'pointer', borderBottom: filter === f ? '2px solid #6366f1' : '2px solid transparent', fontWeight: filter === f ? '600' : '400', background: filter === f ? (t.bg === '#f8fafc' ? 'rgba(99,102,241,0.08)' : 'rgba(99,102,241,0.1)') : 'transparent', borderRadius: filter === f ? '6px 6px 0 0' : '0', textTransform: 'capitalize' }}>
