      ...
      <div style={{ padding: '14px' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', marginBottom: '14px', flexWrap: 'wrap' }}>
          {['All', 'Deposit', 'Withdrawal', 'Completed', 'Pending', 'Failed'].map(f => (
            <button type="button" key={f} onClick={() => { setFilter(f); setPage(1); }} style={{ padding: '4px 10px', background: filter === f ? (t.bg === '#eef0f4' ? 'rgba(99,102,241,0.15)' : '#6366f1') : t.subtleBg, border: filter === f ? '1px solid rgba(99,102,241,0.4)' : `1px solid ${t.border}`, color: filter === f ? '#6366f1' : t.subText, borderRadius: '6px', fontSize: '8px', fontWeight: '600', cursor: 'pointer' }}>{f}</button>
          ))}
        </div>
